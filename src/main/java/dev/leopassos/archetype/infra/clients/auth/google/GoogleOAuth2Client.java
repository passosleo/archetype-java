package dev.leopassos.archetype.infra.clients.auth.google;
...
import dev.leopassos.archetype.infra.dtos.auth.google.GoogleAccessTokenDTO;
import dev.leopassos.archetype.infra.dtos.auth.google.GoogleUserEmailDTO;
...
public class GoogleOAuth2Client implements IOAuth2Client {
    ...
    public String getAccessToken(OAuth2CredentialsDTO credentials) {
        try {
            ...
            if (response.statusCode() == HttpStatus.OK.value()) {
                GoogleAccessTokenDTO data = objectMapper.readValue(response.body(), new TypeReference<>() {
                });
    ...
    public String getUsername(String accessToken) {
        try {
            ...
            if (response.statusCode() == HttpStatus.OK.value()) {
                GoogleUserEmailDTO data = objectMapper.readValue(response.body(), new TypeReference<>() {
                });
